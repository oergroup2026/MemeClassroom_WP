...
import {
  ...
  FileText, MessageSquare, Video, Edit, CheckCircle2, AlertCircle, ShieldAlert
} from "lucide-react";
...
const PendingApprovalPopup = ({ onClose }) => (
  <div className="fixed inset-0 bg-black/50 z-[150] flex items-center justify-center p-4" onClick={onClose}>
    <div className="bg-white dark:bg-zinc-900 border border-yellow-200 dark:border-yellow-800 rounded-2xl shadow-2xl p-6 max-w-sm text-center space-y-3" onClick={e => e.stopPropagation()}>
      <div className="w-12 h-12 mx-auto rounded-full bg-amber-100 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 flex items-center justify-center">
      ...
      <button onClick={onClose} className="bg-ruby-600 hover:bg-ruby-700 text-white font-bold px-6 py-2 rounded-xl text-xs uppercase tracking-wider transition">
        Got it
      </button>
    </div>
  </div>
...
// ─── Collapsible Section ──────────────────────────────────────────────────────
const CollapsibleSection = ({ title, icon: IconComponent, count, defaultOpen = false, children, accentColor = "ruby" }) => {
  const [open, setOpen] = useState(defaultOpen);
  const colors = {
    ruby: "text-ruby-600 dark:text-ruby-400 border-ruby-100 dark:border-ruby-900/40",
    ...
    green: "text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/40",
  };
  return (
...
const ReferenceItem = ({ ref: r, navigate }) => {
  if (r.type === "external") {
    return (
      <a href={r.url} target="_blank" rel="noreferrer"
        className="flex items-center gap-2 text-xs font-bold text-ruby-600 dark:text-ruby-400 hover:underline">
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline text-left"
    >
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto py-16 flex flex-col items-center gap-4 text-gray-400">
        <div className="w-10 h-10 border-4 border-ruby-600 border-t-transparent rounded-full animate-spin" />
  ...
  if (notFound) {
    return (
      <div className="max-w-4xl mx-auto py-24 text-center space-y-4">
        ...
        <p className="text-xs text-gray-500">This activity may have been removed or the link is incorrect.</p>
        <button onClick={() => navigate("/resources?tab=activity")}
          className="bg-ruby-600 hover:bg-ruby-700 text-white font-bold px-5 py-2 rounded-xl text-xs uppercase tracking-wider transition">
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-6">
      ...
      {showFlagConfirm && (
        <div className="fixed inset-0 bg-black/50 z-[150] flex items-center justify-center p-4" onClick={() => setShowFlagConfirm(false)}>
          <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-2xl shadow-2xl p-8 max-w-sm text-center space-y-3" onClick={e => e.stopPropagation()}>
            <ShieldAlert className="w-10 h-10 mx-auto text-orange-500" />
            <h3 className="text-base font-extrabold text-gray-900 dark:text-white">Report Submitted</h3>
            <p className="text-xs text-gray-500 leading-relaxed">Thank you for reporting. Our admin team will review this content.</p>
            <button onClick={() => setShowFlagConfirm(false)} className="bg-ruby-600 hover:bg-ruby-700 text-white font-bold px-6 py-2 rounded-xl text-xs uppercase tracking-wider transition">Got it</button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-2 text-xs font-extrabold text-gray-500 dark:text-gray-400 hover:text-ruby-600 dark:hover:text-ruby-400 transition"
        >
        ...
        <div className="flex items-center gap-2">
          {user && (activity.author_id === user.uid || isAdmin) && (
            <button onClick={() => setShowEditModal(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-ruby-300 dark:border-ruby-800 text-xs font-bold text-ruby-600 dark:text-ruby-400 hover:bg-ruby-50 dark:hover:bg-ruby-950/30 transition"
            >
              <Edit className="w-3.5 h-3.5" /> Edit Activity
            </button>
          )}
          <button onClick={handleLike}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-bold transition ${isLiked ? "border-red-200 dark:border-red-800 text-red-500 bg-red-50 dark:bg-red-950/20" : "border-gray-300 dark:border-zinc-700 text-gray-500 hover:border-red-300 hover:text-red-500"}`}
          >
      ...
      <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="flex flex-col sm:flex-row gap-0">
          <div className="sm:w-56 flex-shrink-0 bg-gradient-to-br from-ruby-100 to-pink-100 dark:from-ruby-950/30 dark:to-zinc-800 flex items-center justify-center" style={{ minHeight: 160 }}>
            {activity.cover_image_url ? (
              <img src={activity.cover_image_url} alt={activity.title}
                className="w-full h-full object-cover sm:max-h-56" />
            ) : (
              <BookOpen className="w-12 h-12 text-ruby-400/50" />
            )}
          </div>
          <div className="flex-1 p-5 space-y-3">
            {isPending && (
              ...
              >
                ...
                Pending Admin Approval — click to review status
              </button>
            )}
            ...
            {(activity.strategy_tags || []).length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {activity.strategy_tags.map(tag => (
                  <span key={tag} className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-ruby-100 dark:bg-ruby-950/40 text-ruby-700 dark:text-ruby-300 border border-ruby-200 dark:border-ruby-800">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
              ...
              {activity.duration_minutes && (
                <span className="flex items-center gap-1 bg-ruby-50 dark:bg-ruby-950/30 text-ruby-700 dark:text-ruby-300 px-2 py-0.5 rounded-full font-bold border border-ruby-100 dark:border-ruby-900">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <div className="w-6 h-6 rounded-full bg-ruby-100 dark:bg-ruby-950 text-ruby-700 dark:text-ruby-300 flex items-center justify-center font-extrabold text-[10px]">
            ...
            {activity.body && (
              <FormattedText text={activity.body} className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed" />
            )}
          </div>
        </div>
      </div>
      ...
      {activity.educator_notes && (
        <CollapsibleSection title="Educator Notes" icon={FileText} accentColor="green" defaultOpen>
          <FormattedText text={activity.educator_notes} className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed" />
        </CollapsibleSection>
      )}
        ...
        <div className="space-y-4">
          ...
          {user ? (
            <form onSubmit={handleCommentSubmit} className="flex gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-zinc-800">
              ...
              <div className="flex-1 flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-gray-300 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-ruby-500"
                />
                <button type="submit" disabled={!commentText.trim() || commentLoading}
                  className="px-4 py-2 bg-ruby-600 hover:bg-ruby-700 text-white text-xs font-bold rounded-xl transition disabled:opacity-40">
                  {commentLoading ? "..." : "Post"}
                </button>
              </div>
            </form>
            ...
            <p className="text-xs text-center text-gray-400 pt-4 border-t border-gray-100 dark:border-zinc-800">
              <a href="/auth" className="text-ruby-600 hover:underline font-bold">Sign in</a> to leave a comment
            </p>
          )}
        </div>
      </CollapsibleSection>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {related.map(r => (
              <button
                ...
                className="flex flex-col items-start gap-2 p-4 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl hover:border-ruby-300 dark:hover:border-ruby-700 hover:shadow-md transition text-left"
              >
                {r.cover_image_url ? (
                  <img src={r.cover_image_url} alt={r.title}
                    className="w-full h-24 object-cover rounded-xl" />
                ) : (
                  <div className="w-full h-24 bg-gradient-to-br from-ruby-100 to-pink-100 dark:from-ruby-950/30 dark:to-zinc-800 rounded-xl flex items-center justify-center text-3xl opacity-30">
                    <BookOpen className="w-8 h-8 text-ruby-500" />
                  </div>
                )}
                ...
                {(r.strategy_tags || []).length > 0 && (
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-ruby-50 dark:bg-ruby-950/30 text-ruby-600 dark:text-ruby-400">
                    {r.strategy_tags[0]}
                  </span>
                )}
              </button>
